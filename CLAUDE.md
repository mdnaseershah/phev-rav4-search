...
The ranked table in the email (`generate_email_html` → `listing_row`) has columns: **Rank, Vehicle, Price, Mileage, Description, Source** (Sunroof is no longer its own column — it's folded into Description, see below). The **Source** shows the marketplace name for marketplace listings (AutoTrader, CarGurus, Kijiji, …) and the **dealership name** for dealer listings (e.g. "Rallye Mitsubishi") — taken from the listing's `source` field when present, otherwise inferred from the URL.
