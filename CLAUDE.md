...
2. **AutoTrader.ca** (`parse_autotrader_api`) — Warms a `requests` session against the search URL (for cookies), then POSTs a JSON payload to `https://www.autotrader.ca/Refinement/Search`. The payload sets `Proximity: -1` (National) so the search is nationwide. The response's `AdsHtml` fragment (rendered listing cards) is parsed by `_parse_autotrader_ads_html` for `/a/…` detail links. Uses `autotrader_model` from config. Uses `_get_mileage_cap(vehicle_config, year)` for mileage filtering. **Fallback:** if the API returns nothing and Playwright is available, falls back to the old headless render + `parse_autotrader_listings`.
   - **Proximity-vs-odometer mileage bug (fixed):** AutoTrader search cards show a proximity distance ("*N* km away" = how far the seller is from the search `Address`, which is `Gatineau, QC`). Because the search is National (`Proximity: -1`), that distance is large (e.g. "2,825 km away") and appears in the card text *before* the odometer, so `_find_mileage` was returning the distance as the mileage — reporting a real 84,264 km car as "2,825 km". The fix strips any `N km away` phrase in `_find_mileage` before reading the odometer (safe for all callers; no real odometer is written that way). Note this also means such cars are now correctly re-checked against their year mileage cap by `_within_caps` — several that had slipped through on the fake low distance are now legitimately dropped for being over-cap. Fuel-economy "l/100 km" was already ignored (its number is 3 digits, outside the odometer regex).
